...
<script>
	...
	export default {
		...
		methods:{
            ...
            // 获取地址
            getAddressList(){
                this.queryUrl =this.deliveryTypeId==4? 'api/myOneslft/getCustomerAddrList':'api/bmallpickuppoint/page'
			...
			// 获取字典数据
			getDictData(){
				this.$http('api/dictData/dictDataList',{dictType:'delivery_type'}).then(res=>{
					this.columns=res
					res.forEach(item=>{
						if(item.dictValue==4){
