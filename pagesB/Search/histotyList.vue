<template>
	<view class="history_list mt40">
		...
		<view v-if='list.length>0' class="list row">
			...
			@click="keyWordHandle(item.keyName)"
			v-for="(item,index) in list" :key='index'>
				<text class="label">{{item.keyName}}</text>
			</view>
		</view>
	...
	export default {
		...
		methods:{
			getData(){
				...
				this.$http('api/searchrecord/getSearchRecordList',data).then(res=>{
					this.list=res.list
				})
			},
			keyWordHandle(key){
				...
				uni.showModal({
					...
					success: (res) => {
						if(res.confirm){
							...
							this.$http('api/searchrecord/deleteSearchRecord',ids,'delete').then(res=>{
								this.getData()
							})
						}
					}
