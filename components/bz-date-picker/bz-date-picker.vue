 ...
 * @property {String} default-time 初始选中时间
  ...
  methods: {
    ...
    initTimeValue() {
      ...
      this.hour = this.formatNumber(
        Math.max(dateFormat('h', time), this.minHour)
      )
    ...
    setDates() {
      this.dates = this.generateArray(1, this.days).map((i) => {
        return this.getDate(
          +new Date(this.showTime) + (i - 1) * 24 * 60 * 60 * 1000,
          i - 1
        )
      })
    ...
    verifyValue(value) {
      ...
      let curMinute = this.formatNumber(
        Math.ceil(dateFormat('mm', this.showTime) / this.minuteStep) *
          this.minuteStep
      )
      ...
      const curMinuteIndex = this.minutes.findIndex(
        (i) => i.value === curMinute
      )
